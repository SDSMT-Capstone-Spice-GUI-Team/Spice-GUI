...
from models.component import (
    ComponentData, COMPONENT_TYPES, SPICE_SYMBOLS, TERMINAL_COUNTS,
    DEFAULT_VALUES, TERMINAL_GEOMETRY, _CLASS_TO_DISPLAY, _DISPLAY_TO_CLASS,
)
...
def _generate(components, wires, nodes, terminal_to_node,
              analysis_type="DC Operating Point", analysis_params=None):
...
def _make_3term_circuit(comp_type, comp_id, value):
    ...
    t2n = {
        (comp_id, 0): node_a, (comp_id, 1): node_a,
        ('V1', 0): node_a,
        (comp_id, 2): node_gnd, ('GND1', 0): node_gnd,
        ('V1', 1): node_gnd,
    }
...
def _make_4term_circuit(comp_type, comp_id, value):
    ...
    nodes = [node_a, node_gnd]
    t2n = {
        (comp_id, 0): node_a, (comp_id, 2): node_a,
        ('V1', 0): node_a,
        (comp_id, 1): node_gnd, (comp_id, 3): node_gnd,
        ('GND1', 0): node_gnd, ('V1', 1): node_gnd,
    }
    return components, wires, nodes, t2n
...
def _make_2term_circuit(comp_type, comp_id, value):
    ...
    nodes = [node_a, node_gnd]
    t2n = {
        ('V1', 0): node_a, (comp_id, 0): node_a,
        (comp_id, 1): node_gnd, ('GND1', 0): node_gnd,
        ('V1', 1): node_gnd,
    }
    return components, wires, nodes, t2n
...
class TestComponentRegistration:
    ...
    NEW_TYPES = [
        'BJT NPN', 'BJT PNP', 'MOSFET NMOS', 'MOSFET PMOS',
        'VC Switch', 'Diode', 'LED', 'Zener Diode',
    ]
    ...
    @pytest.mark.parametrize("comp_type,expected_count", [
        ('BJT NPN', 3), ('BJT PNP', 3),
        ('MOSFET NMOS', 3), ('MOSFET PMOS', 3),
        ('VC Switch', 4),
        ('Diode', 2), ('LED', 2), ('Zener Diode', 2),
    ])
    ...
    SERIALIZED_TYPES = [
        ('BJT NPN', 'BJTNPN'), ('BJT PNP', 'BJTPNP'),
        ('MOSFET NMOS', 'MOSFETNMOS'), ('MOSFET PMOS', 'MOSFETPMOS'),
        ('VC Switch', 'VCSwitch'), ('Zener Diode', 'ZenerDiode'),
    ]
...
class TestCircuitModelIntegration:
    ...
    def test_add_bjt_to_model(self):
        model = CircuitModel()
        bjt = ComponentData(
            component_id='Q1', component_type='BJT NPN',
            value='2N3904', position=(0.0, 0.0))
    ...
    def test_add_mosfet_to_model(self):
        model = CircuitModel()
        mos = ComponentData(
            component_id='M1', component_type='MOSFET NMOS',
            value='NMOS1', position=(0.0, 0.0))
    ...
    def test_add_diode_to_model(self):
        model = CircuitModel()
        diode = ComponentData(
            component_id='D1', component_type='Diode',
            value='IS=1e-14 N=1', position=(0.0, 0.0))
    ...
    def test_wire_bjt_creates_node(self):
        ...
        model.add_wire(WireData(
            start_component_id='Q1', start_terminal=0,
    ...
    def test_bjt_circuit_round_trip(self):
        ...
        model.add_component(ComponentData(
            component_id='Q1', component_type='BJT NPN',
            value='2N3904', position=(100.0, 200.0)))
        model.add_component(ComponentData(
            component_id='GND1', component_type='Ground',
            value='0V', position=(0.0, 0.0)))
        model.add_wire(WireData(
            start_component_id='Q1', start_terminal=2,
    ...
    def test_diode_circuit_round_trip(self):
        ...
        model = CircuitModel()
        model.add_component(ComponentData(
            component_id='D1', component_type='Zener Diode',
            value='IS=1e-14 N=1 BV=5.1', position=(50.0, 50.0)))
        model.add_component(ComponentData(
            component_id='V1', component_type='Voltage Source',
            value='10V', position=(0.0, 0.0)))
        model.add_component(ComponentData(
            component_id='GND1', component_type='Ground',
            value='0V', position=(100.0, 100.0)))
        model.add_wire(WireData(
            start_component_id='V1', start_terminal=0,
            end_component_id='D1', end_terminal=0))
        model.add_wire(WireData(
            start_component_id='D1', start_terminal=1,
            end_component_id='GND1', end_terminal=0))
        model.add_wire(WireData(
            start_component_id='V1', start_terminal=1,
            end_component_id='GND1', end_terminal=0))
...
class TestTerminalGeometry:
    ...
    def test_bjt_rotation_transforms_terminals(self):
        ...
        comp_0 = ComponentData(
            component_id='Q1', component_type='BJT NPN',
            value='2N3904', position=(0.0, 0.0), rotation=0)
        comp_90 = ComponentData(
            component_id='Q1', component_type='BJT NPN',
            value='2N3904', position=(0.0, 0.0), rotation=90)
