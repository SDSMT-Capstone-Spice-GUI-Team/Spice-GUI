    ...
    def _draw_grid(self):
        ...
        for i in range(-grid_extent, grid_extent + 1, grid_size):
            ...
            if is_major:
                ...
                _line = self.scene.addLine(i, -grid_extent, i, grid_extent, major_pen)
                # Don't understand need for _grid_items, might show up later on, commented for now
                # self._grid_items.append(_line)

                # Draw Horizontal Line
                _line = self.scene.addLine(-grid_extent, i, grid_extent, i, major_pen)
                # self._grid_items.append(_line)
                ...
                _line = self.scene.addLine(i, -grid_extent, i, grid_extent, minor_pen)
                # self._grid_items.append(_line)

                # Draw Horizontal Line, same as major
                _line = self.scene.addLine(-grid_extent, i, grid_extent, i, minor_pen)
