...
class TestOpAmp:
    def test_opamp_subcircuit(self):
        ...
        node_gnd = NodeData(
            ...
            wire_indices={0, 1, 2},
            is_ground=True, auto_label='0',
        )
        nodes = [node_gnd]
        t2n = {
            ('OA1', 0): node_gnd, ('OA1', 1): node_gnd,
            ('OA1', 2): node_gnd, ('GND1', 0): node_gnd,
        }
        netlist = _generate(components, wires, nodes, t2n)
...
class TestDependentSources:
    def _make_4term_circuit(self, comp_type, comp_id, value):
        ...
        node_a = NodeData(
            ...
            wire_indices={0, 2}, auto_label='nodeA',
        )
        node_gnd = NodeData(
            terminals={(comp_id, 1), (comp_id, 3), ('GND1', 0), ('V1', 1)},
            wire_indices={1, 3, 4}, is_ground=True, auto_label='0',
        )
