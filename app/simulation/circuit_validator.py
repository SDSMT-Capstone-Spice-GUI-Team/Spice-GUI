...
def validate_circuit(components, wires, analysis_type):
    ...
    for comp in components.values():
        ...
        if len(comp_unconnected) == terminal_count:
            errors.append(
                f"{comp.component_id} ({comp.component_type}) has no connections. Connect its terminals to the circuit."
            )
        elif comp_unconnected:
            warnings.append(
                f"{comp.component_id} ({comp.component_type}) has unconnected terminal(s): {comp_unconnected}."
            )
    ...
    if analysis_type == "DC Sweep":
        ...
        if not dc_sources:
            errors.append("DC Sweep requires at least one DC Voltage Source to sweep.")

    if not voltage_sources and not current_sources:
        warnings.append("Circuit has no voltage or current sources. The simulation may not produce meaningful results.")
